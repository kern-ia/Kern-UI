...
export const VIEWS: ViewDef[] = [
  ...
  // Widgets stand for MCP servers, which are tools/skills the agent wires on demand —
  // not a brick of their own. Same registry as the Grimoire, so the same contract unlocks
  // both views.
  { id: 'espace', glyph: '▦', source: { kind: 'no-contract', brick: 'kern-orch' }, onMobile: true },
  ...
  // kern-orch owns the skills registry and exposes it on its CLI, but no contract
  // publishes it to another brick yet.
  { id: 'grimoire', glyph: 'ᛝ', source: { kind: 'no-contract', brick: 'kern-orch' }, onMobile: true },
]
